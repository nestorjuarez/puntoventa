...
class UpdateRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
           ...
           'email' => 'required|email|string|max:200|unique:providers,'.$this->route('provider')->id,
           'cuit' =>'required|string|max:11|min:11|unique:providers,'.$this->route('provider')->id,
           'address' =>'required|string|max:255',
           'phone' =>'required|string|max:9|min:9|unique:providers,'.$this->route('provider')->id
        ];
    }
